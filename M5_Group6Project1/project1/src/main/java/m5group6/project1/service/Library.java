...
public class Library implements LibraryService, LoanPolicy {
    ...
    public Library(BookDAO bookDAO, LoanDAO loanDAO, UserDAO userDAO) {
    ...
    public Library() {
        this(DAOFactory.bookDAO(), DAOFactory.loanDAO(), DAOFactory.userDAO());
    }
    ...
    public boolean isLibraryEmpty(List<Book> allBooks) {    	
    	if (allBooks.size() == 0) {
    ...
    public int checkFreeBookID(List<Book> ignore) {
        try {
            List<Book> all = getAllBooks();
            all.sort(Comparator.comparingInt(Book::getId));
            int counter = 1;
            for (Book b : all) {
    ...
    public boolean checkForAvailableBook(List<Book> ignore) {
        try {
    ...
    public void updateBook(int bookId, String title, String author) {
        try {
            ...
            if (updated) {
                ...
                Book refreshed = bookDAO.findById(bookId);
